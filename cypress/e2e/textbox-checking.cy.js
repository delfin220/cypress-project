...
describe("Check the text box all web elements", () => {
  ...
  it("Make sure all text fields and labels (Full Name, Email, Current Address, Permanent Address) are visible.", () => {
    ...
    //Current Address
    cy.get("#currentAddress-label")
      .should("be.visible")
      .and("have.text", "Current Address");
    cy.get("#currentAddress").should("be.visible");

    //Permanent Address
    cy.get("#permanentAddress-label")
      .should("be.visible")
      .and("have.text", "Permanent Address");
  ...
  it("Verify displayed data matches input values", () => {
    ...
    cy.get("#output #currentAddress")
      .invoke("text")
      .then((text) => {
        expect(text.trim().replace(/\u00a0/g, " ")).to.eq(
          `Current Address :${currentAddress}`
        );
      });

    // I did same here with the Permananet Address field
    cy.get("#output #permanentAddress").should(
      "have.text",
